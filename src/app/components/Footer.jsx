    ...
    <footer className="bg-gray-800 text-white">
      <div className="max-w-screen-2xl mx-auto py-8 px-4 sm:px-8 md:flex md:justify-between md:items-center border-b z-30">
        <div className="mb-4 md:w-1/2 lg:w-1/3">
          ...
          <p>Wanna look affordably good? Get your wears now!</p>
        ...
        <div className="mb-4 md:w-1/2 lg:w-2/3 md:flex md:flex-col md:items-end">
          ...
          <div className="flex items-center gap-2 justify-end">
            <input
              ...
              className={`bg-gray-700 text-white px-4 py-2 rounded-lg placeholder-transition ${inputFocused || email ? 'input-focus' : ''}`}
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              onFocus={() => setInputFocused(true)}
              onBlur={() => setInputFocused(false)}
            />
            <button className="px-4 py-2 bg-magenta text-white rounded-lg hover:scale-105 transition cursor-pointer" onClick={handleSubscribe}>Subscribe</button>
          </div>

    ...
    <div>
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4 p-2">
          {filteredItemsArrays[0].map((item) => (
            <div key={item.id}>
              <Link href={`/shop/${item.id}`}>
                <div className="border p-2">
                  <Image src={item.images[0]} alt={item.title} width={500} height={380} className="w-full" />
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-2 gap-4 justify-center p-2">
          {filteredItemsArrays[2].map((item) => (
            <div key={item.id}>
              <Link href={`/shop/${item.id}`}>
                <div className="border p-2">
                  <Image src={item.images[0]} alt={item.title} width={500} height={380} className="w-full" />
      ...
      <motion.div 
        ...
        viewport={{ once: true, amount: 0.3 }}
      >
        <h1 className='px-4 sm:px-7 text-2xl md:text-4xl mb-3 mt-2 font-bold'>Fashion In Footwear</h1>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 justify-center p-2">
          {filteredItemsArrays[1].map((item) => (
            <div key={item.id}>
              <Link href={`/shop/${item.id}`}>
                <div className="border p-2">
                  <Image src={item.images[0]} alt={item.title} width={500} height={380} className="w-full" />
